...
let cfg =
  List.Assoc.find_exn ~equal:String.equal
    (Sexplib.Sexp.load_sexp_conv_exn default_cfg Cfg.t_of_sexp)
    "GATEIO"
...
let sides =
  let open Kx in
  conv (Array.map ~f:string_of_side) (Array.map ~f:side_of_string) (v sym)

open Gateio

let pairs =
  let open Kx in
  conv (Array.map ~f:Pair.to_string) (Array.map ~f:Pair.of_string_exn) (v sym)

open Gateio_rest

let tradesw =
  Kx.(
    t9 (v timestamp) pairs (v sym) (v guid) (v guid) sides (v sym) (v float)
      (v float))
...
let kx_of_fills trades =
  ...
  List.iteri trades
    ~f:(fun i ({ id; orderid; pair; side; price; qty; time } : trade) ->
      times.(i) <- time;
      ...
      qties.(i) <- qty);
  Kx_async.create
    Kx.(t3 (a sym) (a sym) tradesw)
    ( "upd",
      "trades",
      (times, syms, xchs, ids, oids, sides, ordTypes, pxs, qties) )

let auth = { Fastrest.key = cfg.key; secret = cfg.secret; meta = [] }

let getFills w =
  Fastrest.request ~auth
    (Gateio_rest.trade_history { base = "xtz"; quote = "btc" })
  >>= fun fills ->
...
let getLedgers _w =
  let rec inner _start =
    ...
    Fastrest.request ~auth (Gateio_rest.entries ())
    >>= fun { deposits; withdrawals } ->
    let len = List.(length deposits + length withdrawals) in
    (* Pipe.write w (kx_of_fills fills) >>= fun () -> *)
    Logs_async.app (fun m -> m "Found %d entries" len) >>= fun () ->
    Deferred.List.iter deposits ~f:(fun e ->
        Log_async.app (fun m -> m "%a" pp_entry e))
    >>= fun () ->
    Deferred.List.iter withdrawals ~f:(fun e ->
        Log_async.app (fun m -> m "%a" pp_entry e))
    >>= fun () -> if len > 0 then inner !latestTs else Deferred.unit
  in
  inner Ptime.epoch

let main () =
  Kx_async.with_connection url (fun { w; _ } ->
      getFills w >>= fun () -> getLedgers w)

let () =
  Command.async ~summary:"Gate.io kdb+ compta"
